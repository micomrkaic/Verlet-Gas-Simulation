...
#define PANEL_W        230    // stats panel width — fixed
...
static void draw_panel(SDL_Renderer *rend,int n,
    ...
    int hx=cfg.sim_w+4, hw=PANEL_W-8, hh=120, hy=ty;
    SDL_SetRenderDrawColor(rend,0x06,0x08,0x10,0xFF);
    ...
    // Controls
    ds(rend,"R-RANDOM RESET", px,cfg.sim_h-52,2,0x77,0x88,0xAA);
    ds(rend,"M-MONO  Q-QUIT", px,cfg.sim_h-30,2,0x77,0x88,0xAA);
}
...
int main(int argc,char **argv){
    ...
    const double FIXED_DT = 1.0/120.0;   // fixed physics step (120 Hz)
    bool running=true;
    ...
    while(running){
        ...
        // Draw particles first, then vessel on top — piston rect covers any
        // particle that momentarily escapes above the ceiling
